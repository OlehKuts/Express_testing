        ...
        <>
          ...
          {!startTesting ? (
            <div className="container">
              {!finalAlert ? (
                <>
                  <div className="pre_testing_alert" id="greeting">
                    <div>{stringifiedDate} </div>
                    {greeting}
                  </div>{" "}
                  <div>
                    {" "}
                    <div
                      className="option startBtn"
                      onClick={() => setStartTesting(true)}
                    >
                      Почати
                    </div>
                  </div>
              ...
              {finalAlert && !showBear ? (
                <>
                  <div className="pre_testing_alert">
                    <div>
                      Результат: {points} з {maxPoints} балів
                    </div>
                    <hr />
                    <BoyIcon />
                    {/* <FlowerIcon /> */}
                    <div className="gratitude">{gratitude}</div>
                  </div>
                  <div>
                    <div
                      className="option startBtn"
              ...
              {showBear ? (
                <>
                  <div className="pre_testing_alert">
                    <div className="finalPhrase">{finalPhrase}</div>
                    <BearIcon />
                  </div>
                </>
              ) : null}
          ...
          {startTesting ? (
            <div className="container">
              <Flashcard
                flashcard={mutedQuestions[currentQuestion.id]}
                flip={flip}
              />
              ...
              <div className="nextDiv">
                ...
                >
                  {currentQuestion.id === 4 ? "Завершити" : "Далі"}
                </div>
                <div
                  className="option nextBtn"
                  style={{ display: !showNextBtn ? `block` : `none` }}
